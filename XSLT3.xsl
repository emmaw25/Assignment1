...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//Business//Programme">
        <p>
    ...
    <xsl:template match="//Business//Programme/code">
        Code: <span style="color:green">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    
    <xsl:template match="//Business//Programme/Progtype">
        Programme Type: <span style="color:red">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    
    <xsl:template match="//Business//Programme/title">
        Course Name: <span style="color:blue">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Business//Programme/host">
        Host Department: <span style="color:00ff00">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Business//Programme/NFQ">
        NFQ Level: <span style="color:00ff00">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Business//Programme/delivery">
        Delivery: <span style="color:00ff00">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Business//Programme/semesters">
        Semesters: <span style="color:00ff00">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Business//Programme/credits">
        Credits: <span style="color:00ff00">
            <xsl:value-of select="."/></span>
        <br />
    </xsl:template>
    <xsl:template match="//Engineering//Programme">
        <p>
            <xsl:apply-templates select="//Code"/>
            <xsl:apply-templates select="//Progtype"/>
            <xsl:apply-templates select="//title"/>
